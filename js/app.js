...
const showProducts = (products) => {
  ...
  for (const product of products) {
    ...
    <img class="product-image" src=${image}></img>
      ...
      <p>Ratings: <i class="fas fa-star ratings-color"></i><i class="fas fa-star ratings-color"></i><i class="fas fa-star ratings-color"></i><i class="fas fa-star ratings-color"></i><i class="fas fa-star-half-alt ratings-color"></i> ${product.rating.rate}</p>
      <p>Total Ratings:<i class="fas fa-user-tie"></i>${product.rating.count}</p>
...
let count = 0;
const addToCart = (price) => { //id= ${product.id},
  count = count + 1;
  updatePrice("price", price);

  updateTaxAndCharge();
  updateTotal(); //myChange
  document.getElementById("total-Products").innerText = count;
};

const getInputValue = (id) => {
  const element = document.getElementById(id).innerText;
  const converted = parseFloat(element);//change
  return converted;
};
